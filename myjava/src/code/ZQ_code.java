 ...
 * @Author: zq
 * @CreateDate: 2018/8/14$ 14:34$
 * @Version: 1.0
 *
 * 一次写多个字节：
 *      如果写的第一个字节是整数（0-127），那么显示的时候查询ASCII表
 *      如果写的第一个字节是负数，那第一个字节会和第二个字节，两个字节组成一个中文显示，查询系统默认的编码表（GBK）
 * 各字符集向下兼容ASCII
 *
 ...
 * ASCII 码
 * <p>
 * 学过计算机的人都知道 ASCII 码,总共有 128 个,用一个字节的低 7 位表示,0~31 是控制字符如换行回车删除等;
 * 32~126 是打印字符,可以通过键盘输入并且能够显示出来。
 * <p>
 * ISO-8859-1
 * <p>
 * 128 个字符显然是不够用的,于是 ISO 组织在 ASCII 码基础上又制定了一些列标准用来扩展 ASCII 编码,
 * 它们是 ISO-8859-1~ISO-8859-15,其中 ISO-8859-1 涵盖了大多数西欧语言字符,所有应用的最广泛。
 * ISO-8859-1 仍然是单字节编码,它总共能表示 256 个字符。
 * <p>
 * GB2312
 * <p>
 * 它的全称是《信息交换用汉字编码字符集 基本集》,它是双字节编码,总的编码范围是 A1-F7,其中从 A1-A9 是符号区,
 * 总共包含 682 个符号,从 B0-F7 是汉字区,包含 6763 个汉字。
 * <p>
 * GBK
 * <p>
 * 全称叫《汉字内码扩展规范》,是国家技术监督局为 windows95 所制定的新的汉字内码规范,它的出现是为了扩展 GB2312,
 * 加入更多的汉字,它的编码范围是 8140~FEFE(去掉 XX7F)总共有 23940 个码位,它能表示 21003 个汉字,它的编码是和 GB2312 兼容的,
 ...
 * 在简体中文Windows操作系统中，ANSI 编码代表 GBK 编码;在繁体中文Windows操作系统中，ANSI编码代表Big5;在日文Windows操作系统中，
 * ANSI 编码代表 Shift_JIS 编码。
 ...
 * 这样会增大网络传输的流量,而且也没必要。而 UTF-8 采用了一种变长技术,每个编码区域有不同的字码长度。不同类型的字符可以是由 1~6 个字节组成。
 * <p>
 * UTF-8 有以下编码规则:
